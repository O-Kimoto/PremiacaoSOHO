...
import openpyxl
import os
print(os.path.exists("G:/Meu Drive/Otavio/soho_pontuacao.xlsx"))

# --- Configuration ---
INPUT_FILE_DEFAULT = "soho_pontuacao.xlsx"
...
OUTPUT_FILE = "Premiação SOHO Consolidado.xlsx"
...
def normalize_name(name):
    """Normaliza nomes removendo espaços extras e convertendo para minúsculas."""
    if pd.isna(name):
...
def get_multiplier(regra):
    ...
    if "realta" in regra_str or "desc re-alta" in regra_str: return -2.0
    elif "ativo" in regra_str or "ativo cruzamento" in regra_str: return 1.0
    elif "ativo base" in regra_str: return 0.5
...
def calcular_ugr_desconexão_venda(regra):
    regra_str = str(regra).strip().lower()
    if regra_str.startswith("indicacao"): return 0
    return 1 if ("desconexão" in regra_str or "desc suspensão" in regra_str or "realta" in regra_str) else 0

def calcular_bl_ativa_venda(produto, regra):
    regra_str = str(regra).strip().lower()
    if regra_str.startswith("indicacao"): return 0
    return 1 if pd.notna(produto) and "mbps" in str(produto).lower() and "cancelamento" not in regra_str and "desconexão" not in regra_str and "desc suspensão" not in regra_str and "desc re-alta" not in regra_str else 0

def calcular_bl_cancelada_venda(produto, regra):
    regra_str = str(regra).strip().lower()
    if regra_str.startswith("indicacao"): return 0
    return 1 if pd.notna(produto) and "mbps" in str(produto).lower() and "cancelamento" in regra_str else 0

def calcular_bl_desconectada_venda(produto, regra):
    regra_str = str(regra).strip().lower()
    if regra_str.startswith("indicacao"): return 0
    return 1 if pd.notna(produto) and "mbps" in str(produto).lower() and ("desconexão" in regra_str or "desc suspensão" in regra_str or "realta" in regra_str) else 0
...
def write_calc_line(ws_func, row_func, col_offset, refs_func, label, value_formula, format_str=None, is_label_bold=True, comment=None):
    ...
    if comment:
        value_cell.comment = openpyxl.comments.Comment(comment, "Script")
...
try:
    ...
    if not os.path.exists(INPUT_FILE):
        raise FileNotFoundError(f"Input file '{INPUT_FILE}' not found. Please ensure 'soho_pontuacao.xlsx' is in the same directory as this script or provide the correct path.")
    ...
    if INPUT_SHEET_AUSENCIAS in xls.sheet_names:
        ...
        print(f"Warning: Sheet '{INPUT_SHEET_AUSENCIAS}' not found. 'Status Ausencia', 'Status Atraso', and 'Penalidade ABS' will be set to 0.", file=sys.stderr)
    ...
    if not df_ausencias.empty:
        # Normalizar os nomes das colunas para busca flexível
        original_columns = df_ausencias.columns
        df_ausencias.columns = [col.strip().lower() for col in df_ausencias.columns]
        print(f"Colunas encontradas na aba 'ausencias': {list(df_ausencias.columns)}")

        # Procurar a coluna 'consultor'
        consultor_col = None
        for col in df_ausencias.columns:
            if col == COL_CONS.lower():
                consultor_col = col
                break
        if consultor_col is None:
            raise ValueError(f"Coluna '{COL_CONS}' não encontrada na aba '{INPUT_SHEET_AUSENCIAS}'.")

        # Normalizar os nomes dos consultores na coluna 'consultor'
        df_ausencias[consultor_col] = df_ausencias[consultor_col].apply(normalize_name)
        print(f"Consultores encontrados na aba 'ausencias': {list(df_ausencias[consultor_col])}")

        # Procurar a coluna 'ausencias'
        ausencias_col = None
        for col in df_ausencias.columns:
            if col == COL_AUSENCIAS.lower():
                ausencias_col = col
                break
        if ausencias_col is None:
            print(f"Warning: Column '{COL_AUSENCIAS}' not found. 'Status Ausencia' will be set to 0.", file=sys.stderr)
            ausencias_dict = {normalize_name(cons): 0 for cons in df_soho[COL_CONS]}
        else:
            df_ausencias[ausencias_col] = pd.to_numeric(df_ausencias[ausencias_col], errors="coerce").fillna(0).astype(int)
            ausencias_dict = dict(zip(df_ausencias[consultor_col], df_ausencias[ausencias_col]))
            print(f"Ausências mapeadas: {ausencias_dict}")

        # Procurar a coluna 'atrasos' com variações
        atrasos_col = None
        possible_atrasos_names = ['atrasos', 'atraso', 'atrasos ', 'atraso ']
        for col in df_ausencias.columns:
            if col in possible_atrasos_names:
                atrasos_col = col
                break
        if atrasos_col is None:
            print(f"Warning: Column for 'atrasos' not found (tried {possible_atrasos_names}). 'Status Atraso' will be set to 0.", file=sys.stderr)
            atrasos_dict = {normalize_name(cons): 0 for cons in df_soho[COL_CONS]}
            atrasos_missing = True
        else:
            atrasos_missing = False
            df_ausencias[atrasos_col] = pd.to_numeric(df_ausencias[atrasos_col], errors="coerce").fillna(0).astype(int)
            atrasos_dict = dict(zip(df_ausencias[consultor_col], df_ausencias[atrasos_col]))
            print(f"Atrasos mapeados: {atrasos_dict}")

        # Procurar a coluna 'advertencias'
        advertencias_col = None
        for col in df_ausencias.columns:
            if col == COL_ADVERTENCIAS.lower():
                advertencias_col = col
                break
        if advertencias_col is None:
            ...
            advertencias_dict = {normalize_name(cons): 0 for cons in df_soho[COL_CONS]}
        else:
            df_ausencias[advertencias_col] = pd.to_numeric(df_ausencias[advertencias_col], errors="coerce").fillna(0).astype(int)
            advertencias_dict = dict(zip(df_ausencias[consultor_col], df_ausencias[advertencias_col]))
    ...
    df_combined[COL_CONS] = df_combined[COL_CONS].apply(normalize_name)  # Normalizar nomes dos consultores
    print(f"Consultores na aba 'soho' (normalizados): {list(df_combined[COL_CONS].unique())}")
    print("Data combined.")
    ...
    for coord_name, coord_group in grouped_coord:
        ...
        for superv_name in supervisors_sorted:
            ...
            for consul_name in consultants_sorted:
                ...
                status_atraso_value = atrasos_dict.get(consul_name, 0)
                print(f"Consultor (normalizado): {consul_name}, Status Atraso obtido: {status_atraso_value}")
                comment = "Coluna 'atrasos' não encontrada na aba 'ausencias'" if atrasos_missing else None
                current_row = write_calc_line(ws, current_row, calc_col_offset, cell_refs, "Status Atraso", status_atraso_value, int_number_format, comment=comment)
                status_atraso_ref = cell_refs["Status Atraso"]
                status_ausencia_value = ausencias_dict.get(consul_name, 0)
                print(f"Consultor (normalizado): {consul_name}, Status Ausencia: {status_ausencia_value}")
                ...
                print(f"Consultor (normalizado): {consul_name}, Advertências: {advertencias_value}")
                current_row = write_calc_line(ws, current_row, calc_col_offset, cell_refs, "Número de Advertências", advertencias_value, int_number_format)
                advertencias_ref = cell_refs["Número de Advertências"]
                penalidade_abs_formula = f"=IF({advertencias_ref}>=3,200,IF({advertencias_ref}=2,120,0))"
                current_row = write_calc_line(ws, current_row, calc_col_offset, cell_refs, "Penalidade ABS", penalidade_abs_formula, currency_format)
                ...
                bonus_values_nested_if = (
                    f"IF({cell_refs['BL Válida']}>=30,1000,IF({cell_refs['BL Válida']}>=25,800,"
                    f"IF({cell_refs['BL Válida']}>=23,700,IF({cell_refs['BL Válida']}>=20,600,"
                    f"IF({cell_refs['BL Válida']}>=17,500,IF({cell_refs['BL Válida']}>=15,400,"
                    f"IF({cell_refs['BL Válida']}>=12,200,IF({cell_refs['BL Válida']}>=10,100,0))))))))"
                )
    ...
    for coord_name_sum, coord_group_sum in grouped_summary_coord:
        ...
        for super_name_sum in supervisors_summary_sorted:
            ...
            if supervisor_start_row <= supervisor_end_row:
                ...
                cons_row = write_calc_line(ws_consolidado, cons_row, cons_col_offset + 2, sup_cell_refs, "Total Pontos", f"=SUM({pontos_range})", number_format)
    ...
    for attempt in range(max_attempts):
        try:
            ...
            print("Reminder: Ensure the 'atrasos' column is filled in the 'ausencias' sheet of the input file.")
            break
